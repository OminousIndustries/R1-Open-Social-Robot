...
public class RoboListen : MonoBehaviour
{
    ...
    // Adding a ConcurrentQueue to handle actions that need to be performed on the main thread.
    ...
    public async void ButtonClick()
    {
        ...
        using (var recognizer = new SpeechRecognizer(config))
        {
            ...
            lock (threadLocker)
            {
                message = newMessage;
                Debug.Log(message);
                _mainThreadWorkQueue.Enqueue(() => StartCoroutine(roboLogicInstance.CallOpenAIApi(message)));
                Debug.Log("Sent message to GPT");
